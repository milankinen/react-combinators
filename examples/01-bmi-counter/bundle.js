...
function model(initialHeight, initialWeight) {
  var setHeight = createAction();
  var setWeight = createAction();
  var height = setHeight.$.toProperty(initialHeight);
  var weight = setWeight.$.toProperty(initialWeight);
  var bmi = _baconjs2["default"].combineWith(weight, height, function (w, h) {
    return Math.round(w / (h * h * 0.0001));
  });

  // yeah. model is just a pure factory function that returns plain object
  return { setHeight: setHeight, setWeight: setWeight, height: height, weight: weight, bmi: bmi };
}

function App() {
  // and here we can use the same object like any other object

  var _model = model(180, 80);

  var setHeight = _model.setHeight;
  var setWeight = _model.setWeight;
  var height = _model.height;
  var weight = _model.weight;
  var bmi = _model.bmi;
...
function renderSlider(title, value, setValue, min, max) {
  return _react2["default"].createElement(
    ...
    _react2["default"].createElement("input", { type: "range", min: min, max: max, value: value,
      onChange: function (e) {
